...
function FullscreenCard({
  navigation: {
    state: {
      params: {questionId, offline, userData, query, date},
    },
  },
}) {
  ...
  return (
    <>
      <Background>
        <ScrollView onContentSizeChange={e => console.log(e)}>
          <Post>
            <PostInfo>
              <Photo
                source={{
...
function ReportAndShare({questionId, query, userData, user, goBack}) {
  ...
  const isMine = userData.data.user.id === Number(user.id)
  return (
    <View style={{flexDirection: 'row'}}>
      {!isMine && <TouchableNativeFeedback
        onPress={() => {
          console.log('reported 횟수:', data.question.reported)
